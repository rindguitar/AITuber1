...
class HatenaEntryAdapter:
    ...
    def post(self, title:str, content:str):
        ...
        response = requests.post(self.req_endpoint, auth=HTTPBasicAuth(username=self.hatena_id, password=self.key), headers={'Content-Type':'application/xml'}, data=xml_data)
    ...
    def __to_xml(self, title:str, content:str):
        ...
        titke_elm = SubElement(entry, "title")
        titke_elm.text = title
