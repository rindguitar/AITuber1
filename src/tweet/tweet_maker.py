...
class TweetMaker:
    ...
    def __init__(self):
        ...
        # APIインスタンスの作成
        self.client = tweepy.Client(consumer_key=consumer_key, consumer_secret=consumer_secret, bearer_token=bearer_token,
    ...
    def __create_tweet_text(self) -> Tweet:
        system_prompt = open(
            self.FILE_PATH+"/docs/make_daily_tweet_prompt.txt", "r", encoding="utf-8").read()
        theme = self.__select_theme()
        user_prompt = f"テーマ:{theme}"
        messages = [self.adapter.create_message(
            "system", system_prompt),self.adapter.create_message("user", user_prompt)]
        res: Tweet = self.adapter.create_structured_output(messages, Tweet)
        return res

    def __select_theme(self) -> str:
        themes: list[str] = open(
            self.FILE_PATH+"/docs/tweet_theme.txt", "r", encoding="utf-8").readlines()
